...
export const ContractData = () => {
  ...
  const { data: buidl } = useScaffoldContract({ contractName: "Buidl" });
  console.log("Buidl: ", buidl);

  const { showAnimation } = useAnimationConfig(TotalSupply);
        ...
        <div className="mt-3 flex items-end justify-between">
          ...
          <div className="w-44 p-0.5 flex items-center bg-neutral border border-primary rounded-full">
            <div
              ...
              style={{ animationPlayState: false ? "running" : "paused" }}
            />
          </div>
        </div>
      </div>
